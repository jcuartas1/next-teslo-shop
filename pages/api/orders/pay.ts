...
import { Message } from '../../../interfaces/paypal';
...
async function payOrder(req: NextApiRequest, res: NextApiResponse<Data>) {
  ...
  const { data } = await axios.get<IPaypal.Message>(`${ process.env.PAYPAL_ORDERS_URL }/${ transactionId }`,{
    headers: {
  ...
  if(!dbOrder){
    await db.disconnect();
    return res.status(400).json({ message: 'Orden no existen en nuestra base de datos' });
  }
